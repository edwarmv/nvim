...
local function validate_layout_config(strategy_name, configuration, values, default_layout_config)
  ...
  local get_value = function(k)
    ...
    if M[k] and strategy_name ~= k and type(val) == "table" then
...
M._format = function(name)
  local strategy_config = M._configurations[name]
...
local function make_documented_layout(name, layout_config, layout)
  ...
  M._configurations[name] = layout_config
...
---
M.horizontal = make_documented_layout(
  "horizontal",
...
---
M.center = make_documented_layout(
  "center",
  vim.tbl_extend("error", shared_options, {
...
--- └──────────────────────────────────────────────────┘
--- </pre>
M.cursor = make_documented_layout(
  "cursor",
...
---
M.vertical = make_documented_layout(
  "vertical",
  vim.tbl_extend("error", shared_options, {
    preview_cutoff = "When lines are less than this value, the preview will be disabled",
...
M.flex = make_documented_layout(
  ...
  function(self, max_columns, max_lines, layout_config)
    ...
    if max_columns < flip_columns and max_lines > flip_lines then
      ...
      return M.vertical(self, max_columns, max_lines, layout_config.vertical)
    else
      self.__flex_strategy = "horizontal"
      return M.horizontal(self, max_columns, max_lines, layout_config.horizontal)
    end
  end
)

M.current_buffer = make_documented_layout("current_buffer", {
  -- No custom options.
...
M.bottom_pane = make_documented_layout(
  "bottom_pane",
  vim.tbl_extend("error", shared_options, {
    preview_width = { "Change the width of Telescope's preview window", "See |resolver.resolve_width()|" },
    preview_cutoff = "When columns are less than this value, the preview will be disabled",
  ...
  function(self, max_columns, max_lines, layout_config)
    ...
    -- Height
    prompt.height = 1
    results.height = height - prompt.height - bs - h_space
    preview.height = height + prompt.height - h_space
    ...
    -- Line
    if layout_config.prompt_position == "top" then
      prompt.line = height + h_space - prompt.height
      results.line = height + h_space + prompt.height + bs
      preview.line = prompt.line
...
M._validate_layout_config = validate_layout_config
